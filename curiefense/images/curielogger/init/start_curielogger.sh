...
create_index () {
	...
	curl --silent -X POST "${CURIELOGGER_KIBANA_URL}$PATTERN_PATH" -H 'Content-Type: application/json' -H 'kbn-xsrf: true' -d '{"attributes": {"title": "logstash-*","timeFieldName": "@timestamp"}}'
}
