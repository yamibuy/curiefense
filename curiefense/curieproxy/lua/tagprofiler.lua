...
function eval_section(request_map, section)
  ...
  if section.entries.singles then
    request_map.handle:logDebug(string.format("match_or_list section.singles %s", json_encode(section.entries.singles)))
    local annotation, tags = match_singles(request_map, section.entries.singles)
  ...
  if section.entries.pairs then
    request_map.handle:logDebug(string.format("match_or_list section.entries.pairs %s", json_encode(section.entries.pairs)))
    local annotation, tags = match_pairs(request_map, section.entries.pairs)
    if not annotation then
      m_pairs = false
      if section_relation_and then return false end
    end
  ...
  if section.entries.iprange then
    local within = section.entries.iprange:contains(request_map.attrs.ip)
    if not within then
      m_iprange = false
      if section_relation_and then return false end
    end
  ...
  -- has entries
  if section.entries.negate_singles and next(section.entries.negate_singles) then
    local annotation, tags = match_singles(request_map, section.entries.negate_singles)
    if annotation then
      nm_singles = false
      if section_relation_and then return false end
    end
  ...
  if section.entries.negate_pairs and next(section.entries.negate_pairs) then
    local annotation, tags = negate_match_pairs(request_map, section.entries.negate_pairs)
    if not annotation then
      nm_pairs = false
      if section_relation_and then return false end
    end
  ...
  if section.entries.negate_iprange:len() > 0 then
    local within = section.entries.negate_iprange:contains(request_map.attrs.ip)
    if within then
      nm_iprange = false
      if section_relation_and then return false end
    end
